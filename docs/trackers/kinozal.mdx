...
FlareSolverr/Chromium отдаёт листинг с двойными кавычками у атрибутов (`class="first bg"`, `class="s"`). Парсер принимает и этот HTML, и старый wire-формат (`class='first bg'`, `class=bg`). Пустая «залогиненная» оболочка без таблицы `t_peer` (~15 KB, есть `Выход`, нет `/details.php?id=`) — это stale-вкладка: JacRed ретраит запрос и **не** помечает страницу ParseAllTask как пустую.
