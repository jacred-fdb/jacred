...
- **`GET /api/v1.0/conf`** — identity + apikey probe: `{ jacred: true, configured, apikey, version }` (`?apikey=...` / `X-Api-Key` / Bearer). Full response combinations: [conf-endpoint.md](conf-endpoint.md).
...
- **`GET /cron/knaben/parse`** — свежие раздачи (по умолчанию `from=0`, `size=300`, `pages=1`, `orderBy=date`, `orderDirection=desc`, все TV+Movies категории). Параметры: `from`, `size` (≤300), `pages` (≤10), `query`, `hours`, `orderBy` (`date`|`seeders`|`peers`), `orderDirection` (`desc`|`asc`), `categories` (через запятую). Окно Knaben API: `from + size ≤ 10000`. Parse и backfill оба запрашивают `hide_unsafe=true` и `hide_xxx=true`.
- **`GET /cron/knaben/backfill`** — заполнение архива по листовым подкатегориям `2001000`–`2008000` и `3001000`–`3008000`: сначала `asc` (старые), при достижении 10 000 — встречный `desc` (новые). Состояние: **`Data/temp/knaben_backfill.json`**. Параметры: `pages` (≤10), `size` (≤300), `reset=true` — начать заново. Конец выдачи на `asc` — только при `total.relation=eq` (`from + rawHits >= total.value`); короткий или пустой ответ с `gte` ретраится и не ставит `complete`. Категории ≤10 000 — `complete` за один проход; ≤20 000 — за два; больше 20 000 без пересечения `asc`/`desc` — `partial` (середина недоступна из‑за лимита API). `complete`/`partial` считаются по той же безопасной выдаче (`hide_unsafe=true`), что и parse. После апгрейда со сборки с `hide_unsafe=false` или ложным `complete` один раз вызвать `/cron/knaben/backfill?reset=true`; в crontab `reset=true` не добавлять.
