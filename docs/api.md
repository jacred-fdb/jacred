...
- **`GET /cron/knaben/backfill`** — заполнение архива по листовым подкатегориям `2001000`–`2008000` и `3001000`–`3008000`: сначала `asc` (старые), при достижении 10 000 — встречный `desc` (новые). Состояние: **`Data/temp/knaben_backfill.json`**. Параметры: `pages` (≤10), `size` (≤300), `reset=true` — начать заново. Категории ≤10 000 — `complete` за один проход; ≤20 000 — за два; больше 20 000 — `partial` (середина недоступна из‑за лимита API). `complete`/`partial` считаются по той же безопасной выдаче, что и parse. После апгрейда со сборки, где backfill шёл с `hide_unsafe=false`, один раз вызвать `/cron/knaben/backfill?reset=true`; в crontab `reset=true` не добавлять.
